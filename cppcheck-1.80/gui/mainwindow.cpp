...
Library::Error MainWindow::loadLibrary(Library *library, QString filename)
{
	...
	// add sj used test 
//	QMessageBox::information(this, tr("Information"), tr("application file path: %1").arg(appPath));
	...
	{
		// add sj used test 
// 		if (Library::ErrorCode::OK != ret.errorcode)
// 		{
// 			QMessageBox::information(this, tr("Information"), tr("errorcode: %1").arg(ret.errorcode));
// 		}
		return ret;
	}
...
void MainWindow::newProjectFile()
{
    ...
    setPath(SETTINGS_LAST_PROJECT_PATH, filepath); // 设置最后一次操作工程路径，并保存注册表
