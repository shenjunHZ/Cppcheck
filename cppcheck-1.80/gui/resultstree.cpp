...
void ResultsTree::startApplication(QStandardItem *target, int application)
{
    ...
    if (target && application >= 0 && application < mApplications->getApplicationCount() && target->parent())
    {
		...
		// add by sj
		int iLine = data["line"].toInt();
		if (iLine < 0)
		{
			iLine = 0;
		}
		QString strLineNum = QString("%1").arg(iLine);
		QString strLine = "-n" + strLineNum;
		...
		bool bNotepad = program.contains("notepad++.exe");
		QString strTemp = "";
		if (bNotepad)
		{
			strTemp = QString("%1 %2 %3").arg(program).arg(params).arg(strLine);
		}
		else
		{
			strTemp = QString("%1 %2 %3").arg(program).arg(params);
		}
        const QString cmdLine = strTemp;
